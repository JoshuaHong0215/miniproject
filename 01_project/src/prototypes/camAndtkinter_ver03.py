...
# ===== Tripline 설정(기준선) =====
TRIPLINE_Y_NORM = 0.45      # ★ 라인을 위로(값을 작게) 이동: OUT=위, IN=아래
TRIPLINE_DEBOUNCE = 0.2     # 초, 교차 판정 디바운스
TRIPLINE_POST_DELAY = 0.5   # 초, 교차 직후 소거 안정화(보수적 버퍼)
OUT_CLEAR_SEC = 3.0         # ★ OUT 구역 무감지 유지 시간(요구사항)
...
class Shared:
    ...
    # ---- Tripline/방향 기반 단축용 ----
    tripline_y_norm: float = TRIPLINE_Y_NORM
    last_in2out_ts: float = 0.0          # IN(아래) → OUT(위)로 넘어간 마지막 시각
    last_out_seen_ts: float = 0.0        # OUT(위) 구역에서 마지막으로 사람이 보인 시각
...
class DoorController:
    ...
    # last_in2out_ts: 문 연 뒤 IN→OUT 교차 시각
    # last_out_seen_ts: OUT 구역 마지막 감지 시각
    # other_calls_waiting: 다른 층 콜 존재 여부
    def update(self, now: float, person_present: bool,
               last_in2out_ts: float, last_out_seen_ts: float,
               other_calls_waiting: bool):
        remain = 0.0
        self.mode_text = ""
        ...
        if self.state == self.OPENING:
            ...
            # ★ 단축 조건
            crossed_after_open = (last_in2out_ts >= self.phase_start)  # 문 열린 후 IN→OUT 교차 있었는가
            out_cleared = (now - max(last_out_seen_ts, last_in2out_ts)) >= OUT_CLEAR_SEC  # OUT에서 3초간 무감지
            cond_short = (not self.seen_in_hold) and crossed_after_open and out_cleared and other_calls_waiting

            if cond_short:
                if (now - last_in2out_ts) >= TRIPLINE_POST_DELAY:
                    self.hold_target = HOLD_SHORT
                    self.mode_text = "MODE: 4/3/4 (IN→OUT & OUT clear & calls waiting)"
                else:
                    self.hold_target = HOLD_DEFAULT
                    self.mode_text = "MODE: 4/7/4 (stabilizing)"
            else:
                self.hold_target = HOLD_DEFAULT
                self.mode_text = "MODE: 4/7/4" + (" (waiting but not cleared)" if other_calls_waiting else " (no extra calls)")
...
class DoorSimApp:
    def __init__(self, root, door: DoorController, shared: Shared):
        ...
        self.active_call: Optional[Tuple[int, Optional[str]]] = None
    ...
    def build_ui(self):
        ...
        # 왼쪽: Camera + Doors
        left = tk.Frame(body); left.pack(side="left", padx=(0,12), anchor="n")
        ...
        self.cam_box.pack(pady=(4, 8)); self.cam_box.pack_propagate(False)
        self.cam_label = tk.Label(self.cam_box, bg="black"); self.cam_label.pack(fill="both", expand=True)
        self.canvas = tk.Canvas(left, width=self.canvas_w, height=self.canvas_h, bg="#111")
        self.canvas.pack(anchor="w")

        # 오른쪽: Floor | Hall | Concept
        right = tk.Frame(body); right.pack(side="left", anchor="n")
        rail = tk.Frame(right); rail.pack(pady=(0,0), fill="x")
        ...
        for fl in range(self.top_floor, self.bottom_floor - 1, -1):
            ...
            self.hall_up_btns[fl] = up_btn; self.hall_dn_btns[fl] = dn_btn
            row += 1
    ...
    def on_close(self):
        stop_event.set(); self.root.destroy()
    ...
    def _update_car_visual(self):
        cx = self._shaft_x; cy = self._floor_to_y(self.car_pos_f)
    ...
    def start_move_to(self, target_floor: int):
        ...
        if int(round(self.car_pos_f)) == target_floor:
            self.door.start_cycle(time.time()); return
        self.car_target = target_floor

    # ---------- 버튼 표시/원복 ----------
    def _mark_floor_button(self, floor: int):
        btn = self.floor_btns.get(floor)
        if btn is not None: btn.configure(background="red", state="disabled")
    ...
    def add_call(self, floor: int):
        ...
        for f, d, _ in self.pending_calls:
            if f == floor and d is None: return
        if self.active_call and self.active_call == (floor, None): return
        self.pending_calls.append((floor, None, time.time()))
        self._mark_floor_button(floor); self.update_calls_label()

    # 내림차순 스윕 포함한 다음 목적지 선택
    def select_next_call(self):
        if not self.pending_calls: return None
        cur = float(self.car_pos_f); calls = list(self.pending_calls)

        same = [(f, d, ts) for (f, d, ts) in calls if f == int(cur)]
        if same:
            target = same[0]; self.pending_calls.remove(target); return target
            ...
            if downs_below:
                ...
                for it in list(self.pending_calls):
                    if it[0] == target[0] and it[1] == target[1]:
                        self.pending_calls.remove(it); break
                return target
            else:
                self.down_sweep_active = False
    ...
    def update_moving_target(self):
        if self.car_target is None: return
        going_up = self.car_target > self.car_pos_f
        if going_up:
            higher_down = [f for (f, d, ts) in self.pending_calls if d == DOWN and f > self.car_target]
            if higher_down:
                old_target = int(self.car_target); new_target = max(higher_down)
                need_requeue = True
                for f, d, *_ in self.pending_calls:
                    if f == old_target and (d == DOWN or d is None): need_requeue = False; break
            ...
            if lower_up: self.car_target = min(lower_up)
    ...
    def add_call_dir(self, floor: int, direction: str):
        ...
        if self.active_call and self.active_call == (floor, direction): return
        self.pending_calls.append((floor, direction, time.time()))
        self._mark_hall_button(floor, direction); self.update_calls_label()

    def cleanup_calls(self, timeout_sec):
        now = time.time(); changed = False
        while self.pending_calls:
            f, d, ts = self.pending_calls[0]
            if d is None or now - ts <= timeout_sec: break
            self.pending_calls.popleft(); changed = True
        if changed: self.update_calls_label()

    def get_pending_preview(self, k: int = 8):
        out = []
        for item in list(self.pending_calls)[:k]:
            if len(item) >= 3: f, d, _ = item
            else: f, _ = item; d = None
    ...
    def _clear_calls_on_arrival(self, floor: int):
        ...
        for it in list(self.pending_calls):
            if it[0] == floor:
                self.pending_calls.remove(it); changed = True
        if changed: self.update_calls_label()
        self._restore_floor_button(floor)
    ...
    def tick(self):
        ...
        # 카메라/방향 기반 단축 관련 신호 읽기
        with self.shared.lock:
            person_present   = self.shared.person_present
            last_in2out_ts   = self.shared.last_in2out_ts
            last_out_seen_ts = self.shared.last_out_seen_ts

        other_calls_waiting = self.has_pending_calls()

        state, remain, mode_text = self.door.update(
            now, person_present,
            last_in2out_ts, last_out_seen_ts,
            other_calls_waiting
        )

        if moving and self.car_target is not None:
            state = "MOVING"; remain = move_remain; mode_text = f"to {int(self.car_target)}F"
        ...
        if self.door.state == DoorController.IDLE and self.car_target is None and self.has_pending_calls():
            item = self.select_next_call()
            if item is not None:
                floor, direction, _ = item
                if direction == DOWN:
                    self.down_sweep_active = True   # 내림차순 시작
...
def camera_worker(shared: Shared, stop_event: threading.Event):
    ...
    # IN/OUT 방향 판정용 상태 (OUT=라인 위, IN=라인 아래)
    prev_any_below = False   # 이전 프레임에 IN(라인 아래)에 사람이 있었는가
    last_in2out_ts_local = 0.0
    last_out_seen_ts_local = 0.0
    ...
    while not stop_event.is_set():
        ...
        if results is not None and getattr(results, "boxes", None) is not None and len(results.boxes) > 0:
            for xyxy, conf, cls in zip(results.boxes.xyxy, results.boxes.conf, results.boxes.cls):
                ...
                conf = float(conf); cls_id = int(cls)
                ...
                if cls_id == 0:
                    ...
                    centers_y.append(int((y1+y2)//2))
                if cls_id in CLASS_NAME:
                    kinds.add(name)

        # === 기준선 & IN/OUT 표기 ===
        h, w = frame.shape[:2]
        y_line = int(shared.tripline_y_norm * h)
        cv2.line(frame, (0, y_line), (w, y_line), (0, 255, 0), 2)
        cv2.putText(frame, "TRIPLINE", (10, max(0, y_line-8)),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,255,0), 1, cv2.LINE_AA)
        cv2.putText(frame, "OUT", (w-70, max(18, y_line-10)),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,255,255), 2, cv2.LINE_AA)
        cv2.putText(frame, "IN", (w-60, min(h-10, y_line+24)),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,255,255), 2, cv2.LINE_AA)

        # 판정: any_above = OUT(위), any_below = IN(아래)
        any_above = any(cy < y_line for cy in centers_y)   # OUT
        any_below = any(cy > y_line for cy in centers_y)   # IN

        now_local = time.time()
        crossed_in2out = False
        # IN(아래)에 있었고 이번 프레임에 OUT(위)이면 IN→OUT 교차
        if prev_any_below and any_above and (now_local - last_in2out_ts_local) > TRIPLINE_DEBOUNCE:
            crossed_in2out = True
            last_in2out_ts_local = now_local
        prev_any_below = any_below

        # OUT(위) 구역 감지 시각 갱신
        if any_above:
            last_out_seen_ts_local = now_local

        # 오버레이
        with shared.lock:
            s_state = shared.state; s_remain = shared.remain; s_mode = shared.mode_text
        ...
        with shared.lock:
            ...
            if crossed_in2out:
                shared.last_in2out_ts = last_in2out_ts_local  # ★ IN→OUT 교차만 기록
            shared.last_out_seen_ts = last_out_seen_ts_local # ★ OUT에서 마지막 감지 시각
